...
add_action( 'admin_init', function () {
	...
	foreach ( $posts as $p ) {
		...
		$filepath = $upload_dir['basedir'] . '/2026/06/' . $p['image'];
		if ( file_exists( $filepath ) ) {
			$filetype  = wp_check_filetype( $p['image'], null );
			$attach_id = wp_insert_attachment( array(
				'guid'           => $upload_dir['baseurl'] . '/2026/06/' . $p['image'],
...
add_action( 'admin_init', function () {
	...
	if ( ! current_user_can( 'manage_options' ) || ! isset( $_GET['nevasenda_blog_images'] ) ) {
		return;
	}

	// /wp-admin/?nevasenda_blog_images=1 -> pone imagen destacada a las entradas
	// de blog importadas anteriormente que todavía no la tengan.
	$images = array(
		'Apertura de refugios en los Pirineos: primeros partes de nieve en cotas altas' => 'blog-refugios.jpg',
		'Cómo elegir botas de trekking según el terreno: guía rápida'                    => 'blog-botas.jpg',
		'Crónica: travesía de dos días por el Macizo Central de Picos de Europa'         => 'blog-picos-europa.jpg',
		'Estado de los senderos en Sierra Nevada este fin de semana'                     => 'blog-sierra-nevada.jpg',
		'Quedadas de la comunidad este otoño: calendario y cómo apuntarte'               => 'blog-quedadas.jpg',
	);
	...
	foreach ( $images as $title => $image ) {
		...
		$filepath = $upload_dir['basedir'] . '/2026/06/' . $image;
		if ( ! file_exists( $filepath ) ) {
			continue;
		}
		...
		$attach_id = wp_insert_attachment( array(
			'guid'           => $upload_dir['baseurl'] . '/2026/06/' . $image,
			'post_mime_type' => $filetype['type'],
			'post_title'     => $title,
			'post_status'    => 'inherit',
		), $filepath, $post->ID );
	...
	wp_die( 'Imagen destacada añadida a ' . $updated . ' entrada(s). <a href="' . esc_url( home_url( '/' ) ) . '">Ver web</a>' );
} );
